...
function getLang() {
    return (window.i18n && window.i18n.getLang()) || localStorage.getItem('oropezas_lang') || 'es';
}
...
function renderFeatured(article) {
    ...
    const readMore = getLang() === 'en' ? 'Read more' : 'Leer más';
    container.innerHTML = `
...
async function loadArticles() {
    try {
        ...
        const lang = getLang();
        const response = await fetch(`${API_BASE}/api/articles?lang=${lang}`);
        const data = await response.json();
        let articles = Array.isArray(data.articles) ? data.articles : [];

        // If no articles in selected lang, fall back to all articles
        if (!articles.length && lang !== 'es') {
            const fallback = await fetch(`${API_BASE}/api/articles`);
            const fb = await fallback.json();
            articles = Array.isArray(fb.articles) ? fb.articles : [];
        }
...
document.addEventListener('oropezas:langchange', loadArticles);
